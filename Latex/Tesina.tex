...
\begin{itemize}
    \item \textbf{AlexNet}: Acc $\approx$ 0.45-0.55, Loss elevata (1.5-4.5) e non decresce adeguatamente, ROC troppo vicina alla diagonale $\rightarrow$  Il modello non apprende pattern utili e generalizza molto male, probabilmente a causa dell'architettura troppo profonda per il dataset disponibile.
    \item \textbf{MiniAlexNet}: Acc $\approx$ 0.48-0.56, Loss in calo ma stabile su valori intermedi ($\approx$1.0-1.5), ROC troppo vicina alla diagonale $\rightarrow$ Il modello mostra una leggera capacità predittiva, ma l'accuratezza vicino al caso casuale e la ROC quasi diagonale indicano che la capacità di discriminazione è ancora limitata (architettura probabilmente insufficiente).
    \item \textbf{MiniAlexNetV2}: Acc $\approx$ 0.6-0.9, Loss in discesa valore per il test $\approx$ 1.75 (rischio di overfitting ma resta comunque il caso migliore in generale), ROC nettamente sopra la diagonale $\rightarrow$ \textbf{miglior compromesso capacità/generalizzazione}.
\end{itemize}

\subsection*{Ottimizzazione Iperparametri su MiniAlexNetV2}
\begin{itemize}
    \item \textbf{LR 0.0001}: Fallimentare per tutti i valori di momentum (Acc $\approx$ 0.1-0.4).
    \item \textbf{LR 0.001}: Risultati intermedi; best a Mom 0.9 (Acc $\approx$ 0.7) ma con overfitting.
    \item \textbf{LR 0.003}: \textbf{Migliori risultati}; best a \textbf{Mom 0.6} (Train Acc $>$ 0.9, Test Acc $\approx$ 0.8, Loss train $\approx$ 0.0, test $\approx$ 1.7, ROC eccellenti).
\end{itemize}
...
\begin{itemize}
    \item Train Acc $\to$ 1.0 già a $\sim$400 epoche; Test Acc si stabilizza a \textbf{0.7}.
    \item Train Loss $\to$ 0.0; Test Loss $\to$ 2.5.
...
\caption{Grafici della condizione migliore per la rete MiniAlexNetV2 (Learning Rate: 0.003, Momentum: 0.6, Epoche: 1200)}
