...
In seguito a una fase preliminare di sperimentazione (discussa nel capitolo successivo), è stata selezionata una Convolutional Neural Network (CNN) ispirata ad AlexNet, denominata \textit{AlexNet}. L'architettura è stata progressivamente migliorata mediante l'introduzione di \textit{Batch Normalization} e \textit{Dropout}, al fine di stabilizzare l'addestramento e ridurre il rischio di overfitting. Nel seguito, il modello finale viene indicato come \textbf{MiniAlexNetV2}. La Figura \ref{fig:reti} mostra la rete di partenza (\textit{AlexNet}) e la sua prima evoluzione (\textit{MiniAlexNet}), mentre la Figura \ref{MiniAlexNetV2} riporta l'architettura finale selezionata.
...
\caption{Confronto tra AlexNet e MiniAlexNet}
\label{fig:reti}
\end{figure}
...
In termini operativi, è stata inoltre considerata la seguente variante della funzione di loss, valida per il caso binario:
...
\begin{align*}
    N & \text{ è il numero di campioni,} \\
    y_i & \text{ è la vera etichetta della classe per il campione } i, \\
    \hat{y}_i & \text{ è la probabilità predetta per il campione } i.
\end{align*}
